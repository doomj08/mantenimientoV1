...
const form = ref({nombre_propiedad:'',tipo_articulo_id:null,seccion_formato_id:null,seccion_formato_id:'',errors:[]});
...
const save=()=>{
    form.value.tipo_articulo_id=props.tipo_articulo
    form.value.seccion_formato_id=props.seccion_formato_id

    sendRequestWithFiles('POST',form.value,'api/tipo_articulos/'+props.tipo_articulos+'/campos_propiedades');
    
}

</script>
<template>
    <button @click="showModal" type="button"  class="inline-flex bg-blue-400 items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:ring-primary-300 sm:w-auto dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-700">
    ...
    <Modal :size="size" v-if="isShowModal" @close="closeModal">
      <template #header>
        <div class="flex items-center text-lg">
          Nuevo Campo (Propiedad)
        </div>
      </template>
      <template #body>
        <Input size="sm" v-model="form.nombre_propiedad" label="Nombre interno" :validationStatus="(form.errors.nombre_propiedad?'error':'')">
