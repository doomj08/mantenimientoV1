...
const campos = ref({})
const form = ref({campos:{},articulo_id:props.articulo_id});
...
const save=()=>{
    ...
    sendRequestWithFiles('POST',form.value,'/api/propiedad');
...
const getCampos=async () =>{
        ...
        .then(
            (response) => {
                campos.value=response.data.data.campo_propiedad
...
<template>
    ...
    <Modal :size="size" v-if="isShowModal" @close="closeModal">
      ...
      <template #body>
        <Input v-for="(campo,index) in campos" :key="index" 
              size="sm" 
              v-model="form.campos[campo.id]" 
              :label="campo.nombre_propiedad" 
              >
