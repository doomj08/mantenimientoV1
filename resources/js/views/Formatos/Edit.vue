...
const save=()=>{
    ...
    sendRequestWithFiles('PUT',form.value,'/api/formatos/'+props.id);
...
const getFormato=async () =>{
        let res;
            ...
            await axios(
            {
                method:'GET', url:'api/formatos/'+props.id, data:null,headers:config.headers
            }
        )
        .then(
            (response) => {
                form.value.version=response.data.data.formato.version
                form.value.cliente_id=response.data.data.formato.cliente_id
                clientes.value.cliente_id=response.data.data.clientes
...
<template>
    ...
    <Modal :size="size" v-if="isShowModal" @close="closeModal">
      ...
      <template #body>
        <Input size="sm" class="text-left" v-model="form.version" label="Versión" :validationStatus="(form.errors.version?'error':'')">
        ...
        <Select class="text-left" v-model="form.cliente_id" :options="clientes" label="Clientes" :validationStatus="(form.errors.cliente_id?'error':'')">
