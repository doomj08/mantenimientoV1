...
const save=()=>{
    ...
    sendRequestWithFiles('PUT',form.value,'api/edit_actividad/'+props.ticket_id+'/'+props.actividad_id);
...
const getActividad=async () =>{
        let res;
            ...
            await axios(
            {
                method:'GET', url:'api/tickets/'+props.ticket_id+'/actividad_ticket/'+props.actividad_id, data:null,headers:config.headers
            }
        )
        .then(
            (response) => {
...
<template>
    <button  @click="showModal" type="button" class="w-full inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
