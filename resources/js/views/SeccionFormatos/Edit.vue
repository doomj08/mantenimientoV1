...
const save=()=>{
    ...
    sendRequestWithFiles('PUT',form.value,'/api/seccion_formato/'+props.id);
...
const getArticulo=async () =>{
        let res;
            ...
            await axios(
            {
                method:'GET', url:'/api/seccion_formato/'+props.id, data:null,headers:config.headers
            }
        )
        .then(
            (response) => {
                form.value.seccion=response.data.data.seccion_formato.seccion
                console.log(response)
                res= response.data.status
            }
        )
...
<template>
    ...
    <Modal :size="size" v-if="isShowModal" @close="closeModal">
      ...
      <template #body>
        <Input size="sm" class="text-left" v-model="form.seccion" label="Nombre interno" :validationStatus="(form.errors.seccion?'error':'')">
            <template #validationMessage v-if="form.errors.seccion">
                <ul>
                    <li v-for="(error,index) in form.errors.seccion" :key="index">{{ error }}</li>
                </ul>
            </template>
        </Input>
